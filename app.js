...
let user = null, profile = null, friends = [], requests = [], sentRequests = [], players = [], games = [], hydroBalance = 0;
let avatarItems = [], avatarInventory = [], threeEditor = null, threePlay = null, editorTool = "move";
...
async function loadData(){
  const [p,f,r,s,g,h,ai,inv]=await Promise.all([
    supabase.from("profiles").select("id,username,created_at,avatar").order("username"),
    ...
    supabase.from("friend_requests").select("id,sender_id,receiver_id,status").eq("sender_id",user.id).eq("status","pending"),
    supabase.from("games").select("*").order("created_at",{ascending:false}),
    supabase.from("hydro_accounts").select("balance").eq("user_id",user.id).maybeSingle(),
    supabase.from("avatar_items").select("*").order("category,name"),
    supabase.from("avatar_inventory").select("item_id").eq("user_id",user.id)
  ]);
  if(p.error)throw p.error;
  players=p.data||[]; hydroBalance=h.data?.balance||0; friends=(f.data||[]).map(x=>x.user_id===user.id?x.friend_id:x.user_id); requests=r.data||[]; sentRequests=s.data||[]; games=g.data||[]; avatarItems=ai.data||[]; avatarInventory=inv.data||[];
}
function nav(view){
...
function renderPlatform(){
  ...
  <section class="view" id="v-profile"><div class="title">Profile</div><div class="subtitle" id="profileSubtitle">Your public Waterloo identity.</div><div id="profileContent"></div></section>
  ...
  renderGames(); renderPlayers(); renderFriends(); wireEditor(); refreshStatus(); renderOwnProfile();
}
...
function renderPlayers(filter=""){const arr=players.filter(p=>p.id!==user.id&&p.username.toLowerCase().includes(filter.toLowerCase()));$("#playersList").innerHTML=arr.length?arr.map(p=>{const isFriend=friends.includes(p.id), outgoing=sentRequests.some(r=>r.receiver_id===p.id);return `<div class="userrow"><div class="miniavatar"></div><div class="grow"><b>${escapeHtml(p.username)}</b><small>Registered Waterloo user</small></div><button class="smallbtn" data-profile="${p.id}">View profile</button>${isFriend?'<span class="online">friends</span>':outgoing?'<span class="online">request sent</span>':`<button class="smallbtn blue" data-add="${p.id}">Add friend</button>`}</div>`}).join(""):`<div class="empty">No other registered users.</div>`;$("[data-add]").forEach(b=>b.onclick=()=>sendFriend(b.dataset.add));$("[data-profile]").forEach(b=>b.onclick=()=>viewProfile(b.dataset.profile));}
$("#app");
async function sendFriend(id){const {error}=await supabase.rpc("send_friend_request",{p_receiver_id:id});if(error){alert("Friend request failed: "+error.message);return;}await loadData();renderPlatform();nav("players")}
async function acceptFriend(id){const {error}=await supabase.rpc("accept_friend_request",{p_request_id:id});if(error){alert("Could not accept request: "+error.message);return;}await loadData();renderPlatform();nav("friends")}
function avatarMarkup(p,large=false){const a=p?.avatar||{},find=id=>avatarItems.find(i=>i.id===id),shirt=find(a.shirt),pants=find(a.pants),hair=find(a.hair);const shirtColor=shirt?.asset?.color||"#2877b8",pantsColor=pants?.asset?.color||"#2c3440",hairColor=hair?.asset?.color||"#3b2a20";return `<div class="avatar-character ${large?"large":""}"><div class="av-hair" style="background:${hairColor}"></div><div class="av-head"></div><div class="av-body" style="background:${shirtColor}"></div><div class="av-pants" style="background:${pantsColor}"></div></div>`}
async function viewProfile(id){const p=players.find(x=>x.id===id)||(await supabase.from("profiles").select("id,username,created_at,avatar").eq("id",id).maybeSingle()).data;if(!p)return;const isFriend=friends.includes(p.id),outgoing=sentRequests.some(r=>r.receiver_id===p.id);nav("profile");$("#profileSubtitle").textContent="Public profile • registered Waterloo user";$("#profileContent").innerHTML=`<div class="profile"><div class="profilebox avatar-card">${avatarMarkup(p,true)}</div><div><div class="profilebox"><h2 style="font-weight:400;margin:0">${escapeHtml(p.username)}</h2><p style="font-size:11px;color:#71808a">Waterloo creator</p><p style="font-size:12px">Member since ${new Date(p.created_at).toLocaleDateString()}</p>${p.id!==user.id?(isFriend?'<span class="online">Friends</span>':outgoing?'<span class="online">Friend request sent</span>':`<button class="smallbtn blue" id="profileAddFriend">Add friend</button>`):''}</div></div></div>`;$("#profileAddFriend")?.addEventListener("click",()=>sendFriend(p.id))}
async function saveAvatar(field,value){const next={...(profile.avatar||{}),[field]:value};const {data,error}=await supabase.from("profiles").update({avatar:next}).eq("id",user.id).select().single();if(error){alert(error.message);return;}profile=data;const i=players.findIndex(p=>p.id===user.id);if(i>=0)players[i]=data;renderOwnProfile()}
function renderOwnProfile(){const a=profile.avatar||{},owned=new Set(avatarInventory.map(x=>x.item_id)),select=(field,cat)=>avatarItems.filter(i=>i.category===cat&&owned.has(i.id)).map(i=>`<option value="${i.id}" ${a[field]===i.id?"selected":""}>${escapeHtml(i.name)}</option>`).join("");$("#profileSubtitle").textContent="Your public Waterloo identity.";$("#profileContent").innerHTML=`<div class="profile"><div><div class="profilebox avatar-card">${avatarMarkup(profile,true)}</div></div><div><div class="profilebox"><h2 style="font-weight:400;margin:0">${escapeHtml(profile.username)}</h2><p style="font-size:11px;color:#71808a">Customize your avatar.</p><div class="avatar-controls"><label>Hair<select id="avHair"><option value="">None</option>${select("hair","hair")}</select></label><label>Face<select id="avFace">${select("face","face")}</select></label><label>Shirt<select id="avShirt">${select("shirt","shirt")}</select></label><label>Pants<select id="avPants">${select("pants","pants")}</select></label></div></div><div class="stats"><div class="stat"><b>${friends.length}</b><span>Friends</span></div><div class="stat"><b>${games.filter(g=>g.owner_id===user.id).length}</b><span>Games</span></div><div class="stat"><b>${scene.objects.length}</b><span>Editor objects</span></div></div></div></div>`;[["avHair","hair"],["avFace","face"],["avShirt","shirt"],["avPants","pants"]].forEach(([id,field])=>$("#"+id)?.addEventListener("change",e=>saveAvatar(field,e.target.value||null)))}
...
document.addEventListener("pointerdown",e=>{if(e.target.matches(".jump")){e.target.setPointerCapture?.(e.pointerId);keys.Space=true}});
document.addEventListener("pointerup",e=>{if(e.target.matches(".jump"))keys.Space=false});
document.addEventListener("pointercancel",e=>{if(e.target.matches(".jump"))keys.Space=false});
function bindJoystick(){
  const joy=$("#joystick"),knob=$("#joyKnob");if(!joy||!knob)return;
  const reset=()=>{knob.style.transform="translate(0,0)";keys.a=keys.d=keys.w=keys.s=false};
  const move=e=>{const r=joy.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2;let dx=e.clientX-cx,dy=e.clientY-cy,max=r.width*.36,len=Math.hypot(dx,dy)||1;if(len>max){dx=dx/len*max;dy=dy/len*max}knob.style.transform=`translate(${dx}px,${dy}px)`;keys.a=dx<-max*.18;keys.d=dx>max*.18;keys.w=dy<-max*.18;keys.s=dy>max*.18};
  joy.onpointerdown=e=>{joy.setPointerCapture?.(e.pointerId);move(e)};joy.onpointermove=e=>{if(joy.hasPointerCapture?.(e.pointerId))move(e)};joy.onpointerup=reset;joy.onpointercancel=reset;
}
bindJoystick();
